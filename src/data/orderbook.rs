...
impl<VUpdate, VStatus: VerificationStatus<VerificationUpdate=VUpdate>> MbpFullbook<VStatus> {
    pub fn update(&mut self, mut update: MbpUpdate<VUpdate>) -> Result<(), crate::util::orderbook::error::OrderbookError> {
...
mod test {
    ...
    mod seqnum_onebyone {
        use super::super::*;
        pub struct SeqNum { pub seqnum: u64 }
    ...
    mod checksum {
        use super::super::*;
        pub struct CheckSum { pub hash: u64 }
        pub fn hashing(depths: &[crate::util::orderbook::depth::MbpDepth]) -> u64 {
            use std::collections::hash_map::DefaultHasher;
            use std::hash::{Hash, Hasher};
            let hash_content = depths.iter().map(|d| d.to_string()).collect::<Vec<String>>().join("");
    ...
    fn test_seqnum_onebyone_success() -> Result<(), Box<dyn Error>> {
        let mut book = MbpFullbook {
            depths: vec![
                MbpDepth { price: dec!(1.0), size: dec!(1.0) },
                MbpDepth { price: dec!(2.0), size: dec!(1.0) },
                MbpDepth { price: dec!(3.0), size: dec!(0.5) },
                MbpDepth { price: dec!(4.0), size: dec!(-1.0) },
                MbpDepth { price: dec!(5.0), size: dec!(-1.0) },
            ],
    ...
    fn test_checksum() -> Result<(), Box<dyn Error>> {
        ...
        let update = MbpUpdate {
            ...
            verification_update: checksum::CheckSum { hash: 15655222248316742011 },
        };
        let rt = book.update(update);
        assert!(rt.is_ok());
        Ok(())
